...
def parse_args():
    ...
    parser.add_argument(
        "--setting",
        help="[gold | open-book | close-book]",
        type=str,
        default="open-book",
    )
    ...
    # parser.add_argument('--corpus_index_path',
    #                     default="../datasets/HOVER/corpus/index", type=str)
    parser.add_argument(
        "--corpus_index_path", default="../datasets/FEVEROUS/corpus/index", type=str
    )
    parser.add_argument("--num_retrieved", default=5, type=int)
    parser.add_argument(
        "--max_evidence_length",
        default=3000,
        help="to avoid exceeding GPU memory",
        type=int,
    )
    parser.add_argument(
        "--my_data",
...
class Program_Execution:
    def __init__(self, args) -> None:
        ...
        if self.args.setting == "open-book":
            self.searcher = PyseriniRetriever(
                self.args.corpus_index_path, use_bm25=True, k1=0.9, b=0.4
            )
        else:
            self.searcher = None
        ...
        # self.gold_evidence_map = {
        #     sample['uid']: sample['supporting_facts'] for sample in dataset}
    ...
    def parse_program(self, ID, program, evidence):
        ...
        for command in program:
            ...
            if c_type == "VERIFY":
                ...
                answer = self.QA_module.answer_verify_question(
                    claim, evidence, claim_only
                )["answer_text"]
                variable_map[return_var] = self.map_direct_answer_to_label(answer)
            # ask a question
            elif c_type == "QUESTION":
                return_var, question = self.parse_question_command(
                    command, variable_map
                )
                # if open-book setting, then retrieve evidence from the corpus
                if self.args.setting == "open-book":
                ...
                answer = self.QA_module.answer_question_directly(
                    question, evidence, claim_only
                )["answer_text"]
    ...
    def execute_on_dataset(self):
        ...
        with open(
            f"./results/programs/{args.dataset_name}_N=1_text-davinci-003_{args.my_data}_programs.json",
            "r",
        ) as f:
            dataset = json.load(f)
        dataset = (
            ...
            else dataset[: self.args.num_eval_samples]
        )
        ...
        for sample in tqdm(dataset):
            ...
            # get evidence
            evidence = (
                self.gold_evidence_map[sample["uid"]]
                if self.args.setting == "gold"
                else None
            )
            ...
            for sample_program in program:
                try:
                    single_prediction, retrieved_evidence = self.parse_program(
                        sample["id"], sample_program, evidence
                    )
            ...
            if args.dataset_name == "HOVER":
                predictions.append(
                    "SUPPORTED" if final_prediction == True else "NOT_SUPPORTED"
                )
                results.append(
                    {
                        "id": sample["id"],
            ...
            if args.dataset_name == "FEVEROUS":
                predictions.append(
                    "SUPPORTS" if final_prediction == True else "REFUTES"
                )
                results.append(
                    {
                        "id": sample["id"],
